...
import { REGIONAL_TECHNICAL_SUPPLIERS } from "./regional-technical-suppliers";
import { REGIONAL_PHOTO_VIDEO_SUPPLIERS } from "./regional-photo-video-suppliers";
...
async function seedRegionalPhotoVideoSuppliers(tenantId: string, organizationId: string) {
  const lastValidatedAt = new Date("2026-08-18T00:00:00.000Z");

  for (const supplier of REGIONAL_PHOTO_VIDEO_SUPPLIERS) {
    const performanceNotes = [
      `Serviços pesquisados: ${supplier.services.join(", ")}.`,
      supplier.notes,
      `Nível de validação: ${supplier.validationLevel}. Status do contato: ${supplier.contactStatus}.`,
  ...
  console.log(`Fornecedores regionais de foto e filmagem: ${REGIONAL_PHOTO_VIDEO_SUPPLIERS.length} registros catalogados.`);
}

async function seedRegionalTechnicalSuppliers(tenantId: string, organizationId: string) {
  const lastValidatedAt = new Date("2026-08-17T00:00:00.000Z");

  for (const supplier of REGIONAL_TECHNICAL_SUPPLIERS) {
    const performanceNotes = [
      `Serviços pesquisados: ${supplier.services.join(", ")}.`,
      supplier.notes,
      `Nível de validação: ${supplier.validationLevel}. Status do contato: ${supplier.contactStatus}.`,
  ...
  console.log(`Fornecedores técnicos: ${REGIONAL_TECHNICAL_SUPPLIERS.length} registros catalogados.`);
}
...
async function main() {
  ...
  await seedRegionalTechnicalSuppliers(tenantId, organizationId);
  await seedRegionalPhotoVideoSuppliers(tenantId, organizationId);
